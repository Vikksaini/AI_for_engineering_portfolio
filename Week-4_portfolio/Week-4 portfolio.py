...
print(df.head())
# Get 333 samples from each class
df_class_0 = df[df['Class'] == 0].sample(333, random_state=10)
df_class_1 = df[df['Class'] == 1].sample(333, random_state=10)
df_class_2 = df[df['Class'] == 2].sample(334, random_state=10)  # taking the remainder

# Combine these samples into one DataFrame
df_test = pd.concat([df_class_0, df_class_1, df_class_2]).sample(frac=1, random_state=10).reset_index(drop=True)
...
print("Here are the constant columns: ", constant_columns)
# Identify columns with few unique integer values (e.g., fewer than 10 unique values)
categorical_col = [col for col in df_train.columns if df_train[col].nunique() < 10 and df_train[col].dtype == 'int64']
...
df_train_balanced.to_csv('vegemite_new_balance.csv', index=False)
...
# Select the features with correlation greater than 0.85
...
df_train_balanced['FFTE Pump 1 - 2 * FFTE Pump 1'] = df_train_balanced['FFTE Pump 1 - 2'] * df_train_balanced['FFTE Pump 1']
...
final_col = original_features + composite_features + ['Class']
# Create the final DataFrame with the specified column order
df_composite_features = df_train_balanced[final_col]
# Save the final DataFrame to a CSV file for future use
df_composite_features.to_csv('vegemitewithcompositefeatures.csv', index=False)
...
extra_features = [
    'FFTE Production solids PV', 'FFTE Discharge solids', 'FFTE Pump 2',
    'FFTE Pump 1', 'FFTE Temperature 2 - 1', 'FFTE Temperature 1 - 1',
    'FFTE Temperature 3 - 2', 'FFTE Pump 1 - 2 * FFTE Pump 1']

# Remove these features from the dataset
df_final = df_composite_features.drop(columns=extra_features)
df_final.to_csv('finalvegemite.csv', index=False)
...
# Initialize the models from the previous studios
X = df_train_balanced.drop(columns='Class')
y = df_train_balanced['Class']
X_train, X_val, y_train, y_val = train_test_split(X, y, test_size=0.3, random_state=1)
models = {
    'SGD': SGDClassifier(random_state=1),
    'Decision Tree': DecisionTreeClassifier(random_state=1), 'SVM': SVC(random_state=1),
    'Random Forest': RandomForestClassifier(random_state=1),
    'MLP Classifier': MLPClassifier(random_state=1, max_iter=500),
}
# Also from this weeks studio tasks
# these are the examples taken from the studio tasks.
...
# Best model from the classification report
...
df_test['FFTE Pump 1 - 2 * FFTE Pump 1'] = df_test['FFTE Pump 1 - 2'] * df_test['FFTE Pump 1']
...
sp_col = [col for col in df_final.columns if col.endswith('SP')]

# Create a new DataFrame with only SP features and the target 'Class'
df_sp = df_final[sp_col + ['Class']]
...
decision_tree_sp = DecisionTreeClassifier(random_state=10)
